...
exec('git remote update').then(gitVerify).catch(async () => {
  ...
  const stable = await SA.get('https://raw.githubusercontent.com/HiekkiStudio/Eclipse/master/package.json')
  logger.log(`Latest version: ${JSON.parse(stable.text).version}`)
