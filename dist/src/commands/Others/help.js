...
module.exports = class extends Command {
    ...
    async run(message, args, lang) {
        ...
        if (args[0]) {
            ...
            embed1.addFields({ name: `${lang.help.outros} (${this.client.commands.filter(command => command.category === "Outros").size})`, value: `${this.client.commands.filter(command => command.category === "Outros").map(e => `\`${this.client.prefix}${e.name}\``).join(" **|** ")}` + '.' }, { name: `${lang.help.musica} (${this.client.commands.filter(command => command.category === "Música").size})`, value: `${this.client.commands.filter(command => command.category === "Música").map(e => `\`${this.client.prefix}${e.name}\``).join(" **|** ")}` + '.' }, { name: `${lang.help.filtros} (${this.client.commands.filter(command => command.category === "Filtros").size})`, value: `${this.client.commands.filter(command => command.category === "Filtros").map(e => `\`${this.client.prefix}${e.name}\``).join(" **|** ")}` + '.' }, { name: `${lang.help.dev} (${this.client.commands.filter(command => command.category === "Desenvolvedor").size})`, value: `${this.client.commands.filter(command => command.category === "Desenvolvedor").map(e => `\`${this.client.prefix}${e.name}\``).join(" **|** ")}` + '.' });
