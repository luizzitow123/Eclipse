...
module.exports = class extends Command {
    ...
    async run(message, args, lang) {
        ...
        if (!player)
            return message.quote(lang.queue.nothing);
        ...
        if (queue.current)
            embed.addField(lang.queue.np, `**[${queue.current.title}](${queue.current.uri})** (${queue.current.requester.tag}) \n↳ ${this.client.utils.time(queue.current.duration)}`);
        if (!tracks.length)
            embed.setDescription(`${lang.queue.nohasmusic} ${page > 1 ? `${lang.queue.arg1} ${page}` : lang.queue.arg2}.`);
        else
            embed.setDescription(tracks.map((track, i) => `\`.${start + (++i)}\` **[${track.title}](${track.uri})** (${track.requester.tag})\n↳ ${this.client.utils.time(track.duration)}`).join("\n"));
